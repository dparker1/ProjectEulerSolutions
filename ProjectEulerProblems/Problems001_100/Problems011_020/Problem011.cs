...
namespace ProjectEulerProblems
{
    public class Problem011
    {
        ...
        {
            int maxProduct = 0, columnProduct = 0, rowProduct = 0, rightDiagonalProduct = 0, leftDiagonalProduct = 0;
            for(int row = 0; row < grid.Length; row++)
            {
                for(int column = 0; column < grid[row].Length; column++)
                {
                    ...
                    {
                        rightDiagonalProduct = grid[row][column] * grid[row + 1][column + 1] * grid[row + 2][column + 2] * grid[row + 3][column + 3];
                    }

                    if(row + 3 < grid.Length && column - 3 >= 0)
                    {
                        leftDiagonalProduct = grid[row][column] * grid[row + 1][column - 1] * grid[row + 2][column - 2] * grid[row + 3][column - 3];
                    }

                    int maxIteration = Enumerable.Max(new int[] { rowProduct, columnProduct, rightDiagonalProduct, leftDiagonalProduct });
                    if(maxIteration > maxProduct)
                    {
                        maxProduct = maxIteration;
                    }
                }
            }
            return maxProduct;
